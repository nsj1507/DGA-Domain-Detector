  ...
  return (
    <div className="space-y-4">
      <section className="rounded-xl border bg-sidebar p-5 text-sidebar-foreground">
        <p className="font-mono text-xs uppercase tracking-[0.16em] text-sidebar-primary">Signal inspection / 01</p>
        <h2 className="mt-3 max-w-2xl text-3xl font-extrabold tracking-[-0.04em]">A readable instrument for uncertain edges.</h2>
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-sidebar-foreground/70">
          DGA Domain Detector pairs warm paper surfaces with deep teal instrumentation and acid-lime signal accents. Technical values stay monospaced; the hierarchy stays calm.
        </p>
      </section>
      ...
      <section className="rounded-xl border bg-card p-5 text-card-foreground">
        <h2 className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Composition principles</h2>
        <div className="mt-4">
          <Guidelines items={[
            { kind: 'do', text: 'Use deep teal for instrument chrome and the primary action.' },
            { kind: 'do', text: 'Use acid lime as a signal, active-state, or positive-verdict accent.' },
            { kind: 'do', text: 'Keep domains, metrics, and system labels in DM Mono for scanability.' },
            { kind: 'dont', text: 'Treat a model probability as calibrated unless held-out calibration exists.' },
          ]} />
        </div>
      </section>
  ...
  return (
    <div className="space-y-8 rounded-xl border bg-card p-6 text-card-foreground">
      ...
      <section className="space-y-4 border-t pt-6">
        <h2 className="font-semibold">Signal semantics</h2>
        <Guidelines items={[
          { kind: 'do', text: 'Reserve muted red for malicious verdicts, errors, and attention states.' },
          { kind: 'do', text: 'Keep the warm paper background visible around card groups to preserve instrument-like separation.' },
          { kind: 'dont', text: 'Use lime as decorative noise; it should indicate activity, emphasis, or positive signal.' },
        ]} />
      </section>
  ...
  return (
    <div className="space-y-8 rounded-xl border bg-card p-6 text-card-foreground">
      ...
      <section className="space-y-4 border-t pt-6">
        ...
        <Guidelines items={[
          { kind: 'do', text: 'Use Manrope for readable UI copy and headings.' },
          { kind: 'do', text: 'Use DM Mono for domain strings, metrics, labels, and model metadata.' },
        ]} />
      </section>
    </div>
  );
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <section className="rounded-xl border bg-card p-6 text-card-foreground lg:col-span-2">
        <h2 className="font-semibold">Surfaces and hierarchy</h2>
        <p className="mt-1 text-sm text-muted-foreground">The application uses a restrained elevation ladder rather than heavy decoration.</p>
        <div className="mt-5">
          <Guidelines items={[
            { kind: 'do', text: 'Group information in soft-radius cards with quiet borders and restrained shadows.' },
            { kind: 'do', text: 'Use a full-width deep teal header for persistent system context.' },
            { kind: 'do', text: 'Keep secondary metadata visually quieter than the verdict and decision score.' },
          ]} />
        </div>
      </section>
    </div>
